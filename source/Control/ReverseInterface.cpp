...
bool ReverseInterface::writeJointCommand(const vector6d_t& pos, ControlMode mode, int timeout) {
    return writeJointCommand(&pos, mode, timeout, BASE_USER_FRAME_ID);
}
...
bool ReverseInterface::writeJointCommand(const vector6d_t& pos, ControlMode mode, int timeout, int32_t user_frame_id) {
    return writeJointCommand(&pos, mode, timeout, user_frame_id);
}
...
bool ReverseInterface::writeJointCommand(const vector6d_t* pos, ControlMode mode, int timeout, int32_t user_frame_id) {
    ...
    data[REVERSE_DATA_SIZE - 2] = htonl(user_frame_id);
    if (pos) {
...
bool ReverseInterface::writeTrajectoryControlAction(TrajectoryControlAction action, int point_number, int timeout) {
    ...
    data[REVERSE_DATA_SIZE - 2] = htonl(BASE_USER_FRAME_ID);
